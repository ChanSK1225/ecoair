...
class CartScreen extends StatelessWidget {
  ...
  Widget _buildCartItem(BuildContext context, dynamic item, StoreProvider provider) {
    return Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('RM ${item.product.price.toStringAsFixed(2)}', style: const TextStyle(color: Color(0xFF0F9D58), fontWeight: FontWeight.bold)),
                const SizedBox(height: 12),
                Row(
                  children: [
                    _buildQtyButton(Icons.remove, () => provider.updateQuantity(item.product.id, -1)),
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 12.0),
                      child: Text('${item.quantity}', style: const TextStyle(fontWeight: FontWeight.bold)),
                    ),
                    _buildQtyButton(Icons.add, () => provider.updateQuantity(item.product.id, 1)),
                  ],
                ),
              ],
            ),
